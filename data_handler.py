from typing import List

import numpy as np
import os
class DataHandler:
    def __init__(self,
                 train: List[str],
                 test: List[str]):
        self.train = train
        self.test = test
        self.classes = [os.path.splitext(os.path.basename(
            e))[0] for e in self.test]
        self.num_classes = len(self.classes)
        self.max_len = 0
        # 5 is the background signal for padding
        self.vocabulary = ['A','C','G','T','N',5]
        self.vocab_size = len(self.vocabulary)
        self.x_train, self.y_train = self.get_data(self.train)
        self.x_test, self.y_test = self.get_data(self.test)
        self.train_size = len(self.y_train)
        self.test_size = len(self.y_test)
        # pads with zero the sequences with length different from max_len
        self.x_train = np.array(
            [np.pad(
                self.x_train[i],
                (0, self.max_len - len(self.x_train[i])),
                'constant',
                constant_values=(0,0)) for i in range(self.train_size)])
        #pads with zero the sequences with length different from max_len
        self.x_test = np.array(
            [np.pad(
                self.x_test[i],
                (0,self.max_len-len(self.x_test[i])),
                'constant',
                constant_values=(0,0)) for i in range(self.test_size)])
    
    def get_data(self, files):
        x = []
        y = []
        converter = {
            'A': 1,
            'C': 2,
            'G': 3,
            'T': 4,
        }
        for fl in files:
            with open(fl,'r') as f:
                cl = self.classes.index(os.path.splitext(
                    os.path.basename(fl))[0])
                for l in f.readlines():
                    if l[0] == '>':
                        if x != []:
                            x[-1] = np.array(
                                [converter.get(c, 5) for c in x[-1]],
                                dtype=np.uint8)

                            if len(x[-1]) > self.max_len:
                                self.max_len = len(x[-1])
                        x.append('')
                        y.append(cl)
                    else:
                        x[-1] += l.upper().strip()
                x[-1] = np.array(
                    [converter.get(c, 5) for c in x[-1]],
                    dtype=np.uint8)
                if len(x[-1]) > self.max_len:
                    self.max_len = len(x[-1])
        return np.array([e for e in x]), np.array([e for e in y])
    
    def get_info_str(self) -> str:
        out = f'{"*" * 79}\n**{" " * 30}  DATASET INFO '\
            f'{" " * 30}**\n{"*" * 79}\n'
        out += f'{"Classes:":20s} {", ".join(self.classes)}\n'
        out += f'{"Train size:":20s} {self.train_size}\n'
        out += f'{"Test size:":20s} {self.test_size}\n'
        out += f'{"Longest sequence:":20s} {self.max_len}\n'
        out += f'{"Vocabulary size:":20s} {self.vocab_size}\n'
        return out